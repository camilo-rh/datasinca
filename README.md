...
**datasinca** es una librería en Python para **descargar, estructurar y analizar** datos históricos de calidad del aire del **SINCA** (Sistema de Información Nacional de Calidad del Aire) del Ministerio del Medio Ambiente de Chile.
...
`ds` es una instancia de la clase `DataSinca` y contiene los siguientes atributos:

#### `ds.data`
es un `pandas.DataFrame` con datos históricos del SINCA.

#### `ds.validacion`
es un `pandas.DataFrame` con el estado de validación de cada dato (aplica **solo a contaminantes**)\
Valores posibles:
- `'validado'`
- `'preliminar'`
- `'novalidado'`
...
- filas con índice temporal (`datetime`)
- columnas con `MultiIndex`: (comuna, estacion, parametro, unidad, altura)


#### Uso mínimo de `DataSinca`
...
Antes de exportar los datos o trabajar directamente con `pandas.DataFrame`, se recomienda aplanar (o reordenar) los niveles de las columnas y conservar solo los metadatos de interés.
```python
...
Los filtros de descarga también se pueden configurar en el constructor y como atributos. De esta manera, la configuración se conserva y permite explorar dinámicamente las opciones disponibles (ver [comportamiento](#comportamiento-del-cliente-de-descarga-sinca)).
...
## Tipos de parámetros y Validación
...
- **Meteorológicos** → **no tienen información de validación**
...
Comportamiento:
- Dataset **mixto** → se consideran solo contaminantes _(warning)_
- Dataset **solo meteorológico** → error
- Datos faltantes (`NaN`) → **no se consideran estados de validación**

*Nota: Las variables meteorológicas mantienen una columna en `ds.validacion` con `NaN` para conservar una estructura consistente del dataset.*
